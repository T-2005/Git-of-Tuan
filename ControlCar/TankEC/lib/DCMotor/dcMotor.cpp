...
void DCMotor::motorControl(int pwmChannel, int in1, int in2, int speed) {
    if (speed == 0) {
...
void DCMotor::drive(int speedL, int speedR) {
    motorControl(_pwmChannelL, _in1L, _in2L, speedL);
    motorControl(_pwmChannelR, _in1R, _in2R, speedR);
}

void DCMotor::stop() {
    motorControl(_pwmChannelL, _in1L, _in2L, 0);
    motorControl(_pwmChannelR, _in1R, _in2R, 0);
}

void DCMotor::motion(int speedL, int speedR) {
    drive(speedL, speedR);
...
float DCMotor :: PID_control(float setValue, float readValue, uint32_t lastTimePID, float Kp, float Ki, float Kd)
{
